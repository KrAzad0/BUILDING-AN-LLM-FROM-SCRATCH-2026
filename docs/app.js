...
const controls = {
  ...
  embd: document.querySelector("#embd")
};
const values = {
  layers: document.querySelector("#layers-v"),
  heads: document.querySelector("#heads-v"),
  embd: document.querySelector("#embd-v")
};
...
function calculateParams() {
  const L = Number(controls.layers.value);
  const H = Number(controls.heads.value);
  const C = Number(controls.embd.value);
  values.layers.textContent = L;
  values.heads.textContent = H;
  values.embd.textContent = C;

  const vocab = 256;
  const context = 128;
  const embeddings = vocab * C + context * C;
  const perBlock = 12 * C * C + 13 * C;
  const finalNorm = 2 * C;
  const approximate = embeddings + L * perBlock + finalNorm;
  const valid = C % H === 0;
